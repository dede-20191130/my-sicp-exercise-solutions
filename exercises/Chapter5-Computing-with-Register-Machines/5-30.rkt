  ...
  (list (list 'car
        ...
        (list '/
              (lambda(x . dividers)
                (let go ((dvs dividers))
                  (if (null? dvs)
                      (apply-in-underlying-scheme / (append (list x) dividers))
                      (if (= (car dvs) 0)
                          '*condition-code-div-0*
                          (go (cdr dvs)))))))
        (list '> >)
        (list '< <)
        (list '= =)
...
(define (primitive-procedures-report-string cond-code)
  (cond ((eq? cond-code '*condition-code-div-0*) 'attempt-to-divide-by-zero-error)
        ((eq? cond-code '*condition-code-not-pair*) 'attempt-to-pair-operation-to-not-pair-val)
        (else '*undefined*)))
...
(define eceval-operations
  (list (list 'eq? eq?)
        ...
        (list 'primitive-procedures-report-string primitive-procedures-report-string)))
  ...
  (make-machine
   ...
   '(preset-reading
     ...
     (assign val
             ...
             (reg exp)
             (reg env))
     (test (op eq?) (reg val) (const *condition-code-unbound-var*))
     (branch (label unbounded-variable))
     (goto (reg continue))
     ev-quoted
     (assign val
     ...
     (assign val (op apply-primitive-procedure)
             (reg proc)
             (reg argl))
     (assign proc (op primitive-procedures-report-string) (reg val))
     (assign argl (op eq?) (reg proc) (const *undefined*))
     (test (op not) (reg argl))
...
(define presets
  '(begin
     (define new-withdraw ;; for let confirmation
       (let ((balance 100))
         (lambda (amount)
           (if (>= balance amount)
               (begin (set! balance 
                      ...
                      balance)
               "Insufficient funds"))))
     (define (weekdays abbr) ;; for cond confirmation
       (cond ((eq? abbr 'mon) 'Monday)
             ((eq? abbr 'tue) 'Tuesday)
             ((eq? abbr 'wed) 'Webnesday)
             (else 'Others)))))
