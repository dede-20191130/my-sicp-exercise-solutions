...
(define (open-code-primitives? exp)
  (cond ((not (pair? exp)) false)
        ((memq (car exp) open-code-primitives) true)
        (else false)))
...
(define (spread-arguments operand-list)
  (let ((operand1 (car operand-list))
    ...
    (let ((compile-operand1 (compile operand1 'val 'next))
          (compile-operand2 (compile operand2 'val 'next)))
      (preserving
       '(env)
       (append-instruction-sequences
        compile-operand1
...
(define cmpled-seq1 (compile
                        '(begin
                           (+ 1 2 (* 3 4 5)))
                        'val
                        'next))

(define cmpled-fcl-iter (compile
                         '(define (factorial n)
...
(statements cmpled-proc1)
(statements cmpled-seq1)

;(statements cmpled-ex5-35)
