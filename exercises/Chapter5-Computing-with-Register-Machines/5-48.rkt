...
(define primitive-procedures
  (list (list 'car car)
        ...
        (list 'sin sin)
        (list 'compile-and-run (lambda(e) (compile-and-run e)))
        ))
...
(define eceval
  (make-machine
   ...
   '((assign compapp (label compound-apply))
     ;     (assign extent (label external-entry))
      ...
      (lambda (expression)
        ...
        (let ((instructions
               (assemble 
                (statements
                 (compile 
                  expression 'val 'next the-global-environment))
                eceval)))
          (let ((saved-pc-contents (get-register-contents eceval 'pc)))
            ((eceval 'redirect) instructions)
            (set-register-contents! eceval 'pc saved-pc-contents)
...
;        1
;        (* (factorial (- n 1)) n)))
;                   'val 'next the-global-environment)))