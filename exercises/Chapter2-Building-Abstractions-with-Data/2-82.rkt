...
(define (apply-generic op . args)
  (define (all-same-type? my-tags val-1st)
    (cond ((null? my-tags) true)
          ...
          (else false)))
  (define (can-coerce-to-tgt-type? my-tags tgt-type)
    (if (null? my-tags)
        true
        (let ((curr (car my-tags)))
          (if (eq? curr tgt-type)
              (can-coerce-to-tgt-type? (cdr my-tags) tgt-type)
              (let ((curr->tgt (get-coercion curr tgt-type)))
                (if curr->tgt
                    (can-coerce-to-tgt-type? (cdr my-tags) tgt-type)
                    false))))))
  (define (coerce-to-tgt-type my-args tgt-type)
    (if (null? my-args)
        nil
        (let ((curr-arg (car my-args)))
          (if (eq? (type-tag curr-arg) tgt-type)
              (cons curr-arg (coerce-to-tgt-type (cdr my-args) tgt-type))
              (cons ((get-coercion (type-tag curr-arg) tgt-type) curr-arg) (coerce-to-tgt-type (cdr my-args) tgt-type))))))
  (define (try-coerce candidates my-tags my-args)
    (cond ((null? candidates) false)
          ((can-coerce-to-tgt-type? my-tags (car candidates)) (coerce-to-tgt-type my-args (car candidates)))
          (else (try-coerce (cdr candidates) my-tags my-args))))
      ...
      (let ((proc (get op type-tags)))
        (if proc
            ...
            (cond ((< (length arg-list) 2) (error "No method for these types"
                                                  (list op type-tags)))
                  ((all-same-type? type-tags (car type-tags)) (error "No method for these types whose types are same" (list op type-tags)))
                  (else
                   (let ((coerce-result (try-coerce type-tags type-tags arg-list)))
                     (if coerce-result
                         (helper coerce-result)
...
(print-line (exp num1 num2))
;(print-line (exp comp1 comp2))
(print-line (add num1 rat1))
(print-line (add rat1 num1))
(print-line (mul comp1 num2))
(print-line (mul num2 comp1))
