...
(define (install-polynomial-package)
  ...
  (define (pseudoremainder-terms L1 L2)
    (let ((leading-coeff (coeff (first-term L2)))
          (od-dividend (order (first-term L1)))
          (od-divider (order (first-term L2))))
      (let ((integerizing-factor (exp leading-coeff (+ 1 od-dividend (- od-divider)))))
        (remainder-of-rst (div-terms
                           (mul-terms
                            L1
                            (make-term-list 'dense (list integerizing-factor)))
                           L2)))))

  (define (remove-common-for-terms tl)
    (define (acc-coeff tgt-tl)
      (if (empty-termlist? tgt-tl)
          nil
          (cons (contents (coeff (first-term tgt-tl))) (acc-coeff (rest-terms tgt-tl)))))
    (let ((coeffs (acc-coeff tl)))
      (let ((gcd-coeffs (apply gcd coeffs)))
        (quotient-of-rst (div-terms tl (make-term-list 'dense (list gcd-coeffs)))))))
  
  (define (gcd-terms L1 L2)
    (if (empty-termlist? L2)
        (remove-common-for-terms L1)
...
(define p1 (make-polynomial-from-dense-list 'x  '(1 0 1)))
(define p2 (make-polynomial-from-dense-list 'x  '(1 0 0 1)))
(define p3 (make-polynomial-from-sparse-list 'x '((4 1) (3 -1) (2 -2) (1 2))))
(define p4 (make-polynomial-from-sparse-list 'x '((3 1) (1 -1))))
(define rf (make-rational-poly p2 p1))

(define p-pseudodiv-1 (make-polynomial-from-dense-list 'x  '(1 -2 1)))
(define p-pseudodiv-2 (make-polynomial-from-dense-list 'x  '(11 0 7)))
(define p-pseudodiv-3 (make-polynomial-from-dense-list 'x  '(13 5)))
(define q-pseudodiv-1 (mul p-pseudodiv-1 p-pseudodiv-2))
(define q-pseudodiv-2 (mul p-pseudodiv-1 p-pseudodiv-3))


(greatest-common-divisor p3 p4)
(greatest-common-divisor q-pseudodiv-1 q-pseudodiv-2)
