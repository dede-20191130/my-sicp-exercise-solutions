    ...
    (if (same-variable? (variable p1) (variable p2))
        (let ((tgt-var (variable p1))
              (div-term-result (div-terms (term-list p1)
                                          (term-list p2))))
          (list (make-poly tgt-var (car div-term-result))
                (make-poly tgt-var (cadr div-term-result))))
  ...
  (put 'div '(polynomial polynomial) 
       (lambda (p1 p2)
         (let ((div-poly-result (div-poly p1 p2)))
           (list (tag (car div-poly-result)) (tag (cadr div-poly-result))))))
...
(define poly-dns-1 (make-polynomial-from-dense-list 'x  '(2.2 0 1.7)))
(define poly-dns-2 (make-polynomial-from-dense-list 'x  '(5 7)))
(define poly-dns-y-1 (make-polynomial-from-dense-list 'y  '(3 4)))
(define poly-dns-y-2 (make-polynomial-from-dense-list 'y  '(2 3)))
(define poly-dns-y-3 (make-polynomial-from-dense-list 'y  '(1 2)))
(define poly-dns-3 (make-polynomial-from-dense-list 'x  (list poly-dns-y-2 poly-dns-y-3)))
(define poly-dns-y-4 (make-polynomial-from-dense-list 'y  (list poly-dns-2 poly-dns-2)))

(define poly-sp-1 (make-polynomial-from-sparse-list 'x (list term-x-3 term-x-2 term-x-1)))
(define poly-sp-2 (make-polynomial-from-sparse-list 'x (list term-x-8 term-x-7 term-x-6)))

(print-line (add poly-dns-3 (make-polynomial-from-dense-list 'x '(7))))
(print-line (add poly-dns-3 poly-dns-y-4))
(print-line (add poly-dns-3 poly-dns-3))
(print-line "----------------")
(print-line (mul poly-dns-3 (make-polynomial-from-dense-list 'x '(7))))
(print-line (mul poly-dns-3 poly-dns-y-4))