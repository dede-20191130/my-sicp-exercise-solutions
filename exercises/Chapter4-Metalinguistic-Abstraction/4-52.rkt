...
(define (analyze-parm-assignment exp)
  (let ((var (assignment-variable exp))
        (vproc (analyze 
                (assignment-value exp))))
    (lambda (env succeed fail)
...
(define (analyze-if-fail exp)
  (let ((evproc (analyze (iffail-evaled exp)))
        (fbproc (analyze (iffail-fallback exp))))
    (lambda (env succeed fail)
      (evproc env
              (lambda (val fail2)
                (succeed val fail2))
...
(define (iffail-evaled exp) (cadr exp))
(define (iffail-fallback exp) (caddr exp))
...
(define presets
  '(begin
     ...
     (define (iffailtest-atleast-one-succeed)
       (if-fail
