...
(define (list->lazylist _list env)
  (if (null? _list)
      '()
      (eval `(cons ',(car _list) ',(cdr _list)) env)))

  
(define (eval-quotation exp env)
  (let ((tq (text-of-quotation exp)))
    (if (list? tq)
        (list->lazylist tq env)
        tq)))
...
(eval conses the-global-environment)
...
(eval presets the-global-environment)
