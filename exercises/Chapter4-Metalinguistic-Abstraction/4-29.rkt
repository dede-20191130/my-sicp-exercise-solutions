...
(define memorizing-switch true)

(define (force-it obj)
  (if memorizing-switch
      (cond ((thunk? obj)
             (let ((result
                    (actual-value 
                     (thunk-exp obj)
...
(define the-second-global-environment 
  (setup-environment))

;time measurement

(define plus-10-code '(define (plus-10 x) (+ x 10)))
(define explicitly-plus-10-code '(define (explicitly-plus-10 y) (+ y y y y y y y y y y)))
(define calc-code '(explicitly-plus-10 (plus-10 3)))

(set! memorizing-switch false)
(eval plus-10-code the-global-environment)
(eval explicitly-plus-10-code the-global-environment)
(time
 (for ((i (in-range 0 10000)))     
   (eval calc-code the-global-environment)))

(set! memorizing-switch true)
(eval plus-10-code the-second-global-environment)
(eval explicitly-plus-10-code the-second-global-environment)
(time
 (for ((i (in-range 0 10000)))     
   (eval calc-code the-second-global-environment)))

;;;repl

;comment out this and start interaction
;;(set! memorizing-switch true)
;;(driver-loop)
